...
fn main() {
    ...
    for (i, (mode, _name)) in modes.iter().enumerate() {
        ...
        circle1.append_circle(x + 70.0, y + 60.0, 45.0, 45.0, true).unwrap();
        ...
        circle2.append_circle(x + 110.0, y + 60.0, 45.0, 45.0, true).unwrap();
    ...
    println!("Blend modes shown: {}", modes.iter().map(|(_, n)| *n).collect::<alloc::vec::Vec<_>>().join(", "));
}
